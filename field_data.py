...
FIELD_ALIASES: list[tuple[str, str, list[str]]] = [
    ...
    ("p_sample", "Давление отбора/исследования", ["давлениеисследования", "давлениеотбора",
                                                   "pressuresample", "pпласт", "pressure"]),
...
def build_clean_dataframe(df_raw: pd.DataFrame, mapping: dict[str, str]) -> pd.DataFrame:
    ...
    Плюс три QC-проверки (см. модуль field_data — они не отбрасывают строки,
    только помечают, решение остаётся за пользователем):
    - is_anomalous       — проба помечена звёздочкой (*) в номере скважины/
                            пробы, как это принято во многих отчётах для
                            "непредставительных" замеров (см. сноску в файле)
    - rs_cross_check_dev_pct — расхождение, %, между Rs(м3/м3) "напрямую" и
                            Rs(м3/т)*плотность_нефти — если в отчёте есть обе
                            единицы газосодержания, это бесплатная проверка
                            согласованности одной и той же пробы
    - p_below_pb          — True, если давление отбора/исследования пробы
                            ниже давления насыщения (означает, что при отборе
                            в пласте уже мог идти двухфазный поток)
    ...
    # QC 1: проба помечена звёздочкой в номере скважины/пробы ("31*", "25*")
    # — распространённое обозначение "аномального"/"непредставительного" замера.
    # Проверяем И "№ пробы", И "скважину" — в разных отчётах звёздочку могут
    # ставить в любой из этих колонок (в исходном примере она была у скважины).
    anomaly_headers = [h for h in (mapping.get("sample_no"), mapping.get("well")) if h]
    if anomaly_headers:
        is_anomalous = pd.Series(False, index=df_raw.index)
        for h in anomaly_headers:
            is_anomalous |= df_raw[h].apply(lambda v: "*" in str(v) if v is not None else False)
        out["is_anomalous"] = is_anomalous
    else:
        out["is_anomalous"] = False

    # QC 2: перекрёстная проверка через материальный баланс. ВАЖНО: нельзя
    # просто сравнивать Rs(м3/м3) с Rs(м3/т)*плотность_ПЛАСТОВОЙ_нефти — это
    # физически неверно и систематически ошибается на 15-30% (проверено),
    # т.к. "м3/т" в таких отчётах считают на тонну ТОВАРНОЙ (сепарированной)
    # нефти, а не пластовой. Правильно — сначала вывести плотность товарной
    # нефти из баланса массы для этой же пробы:
    #     ρ_пласт * Bo = ρ_товарная + Rs[м3/м3] * γg * ρ_воздух_ст
    # (масса в 1 м3 пластовой нефти = масса товарной нефти на выходе +
    # масса выделившегося из неё газа; ρ_воздух_ст ≈ 1.2232 кг/м3 при
    # стандартных условиях 14.696 psi / 60°F — те же условия, что и в
    # pvt_correlations.bg_gas()), а затем сравнить Rs(м3/т)*ρ_товарная
    # с фактическим Rs(м3/м3) для той же пробы.
    RHO_AIR_STD_KG_M3 = 1.2232
    has_massbalance_inputs = all(
        k in out.columns for k in ("rho_oil", "bo", "rs_m3m3", "rs_m3t", "gas_rel_density")
    )
    if has_massbalance_inputs:
        rho_stb_tm3 = (out["rho_oil"] * out["bo"]
                        - out["rs_m3m3"] * out["gas_rel_density"] * RHO_AIR_STD_KG_M3 / 1000.0)
        rs_m3m3_implied = out["rs_m3t"] * rho_stb_tm3
        out["rs_cross_check_dev_pct"] = (
            (out["rs_m3m3"] - rs_m3m3_implied).abs() / out["rs_m3m3"] * 100
        )
    else:
        # без относительной плотности газа (γg) для пробы корректный
        # материальный баланс не построить — честно показываем "нет данных",
        # а не вводящую в заблуждение цифру
        out["rs_cross_check_dev_pct"] = np.nan

    # QC 3: давление отбора ниже давления насыщения — при отборе пробы в
    # пласте уже могла идти двухфазная фильтрация (флаг для внимания инженера)
    if "p_sample" in out.columns and "pb" in out.columns:
        out["p_below_pb"] = out["p_sample"] < out["pb"]
    else:
        out["p_below_pb"] = False
